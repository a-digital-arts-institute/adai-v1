...
## What's in the graph (May 2026 — Seed Canon v2 rebuild)

| | count |
|---|---|
| Nodes | **16,244** across 6 types: artwork (9,972), practitioner (6,249), concept (14), classification_regime (6), platform (2), institution (1) |
| Edges | **50,793** — 5 curated edge types live (CLASSIFIED_BY 20,798 · CREATED_BY 11,582 · EXHIBITED_AT 9,972 · EMBODIES 4,786 · PRACTICES 3,655) + 4 reserved at zero (BELONGS_TO / COLLABORATES_WITH / USES_TECHNIQUE / INFLUENCES) + RESPONDS_TO empty by design (artist-intent only) + 2 auto-derived from embeddings (STYLE_KIN, VISUALLY_AFFINE) |
| Multimodal embeddings | Re-populated by the daily `embed-derive-daily` GitHub Actions workflow against the new canon (Gemini Embedding 2, 768-d) |

**v1 → v2 in May 2026.** v1 (1,491 nodes, broader institutional / scene / collective curation) accumulated LLM-enrichment contamination over several editorial passes — every row was plausibly worded, fewer were verifiable. v2 starts from a source-attested ground-up rebuild: every row traces to a producer (MoMA digital / Wikidata SPARQL / Art Blocks Hasura / fxhash GraphQL) → a source URL → a fetched_at timestamp, validator-enforced (no description field without sibling source URL). The long-tail institutions / collectives / scenes the v1 canon curated are deferred to a post-rebuild curation pass via the contributor API or `find_missing_images.py` discovery. See [`seed/_build/PRODUCER_CONTRACT.md`](seed/_build/PRODUCER_CONTRACT.md) for the contract every gatherer must follow.
...
│   ├── embeddings.bin           — N × 768 f32 LE (regenerated by daily GH Action)
