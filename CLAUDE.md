...
**Daily re-derive on Fly**: `.github/workflows/embed-derive-daily.yml` runs two stages against the live DB at 04:00 UTC — `embed:backfill` (catches anything that slipped past embed-on-write, calls Gemini) and `embed:derive` (propagates curator-approved CREATED_BY edges into STYLE_KIN / VISUALLY_AFFINE / SUGGESTS_CREATED_BY, no API calls) — then `embed:report-drift`. Triggered manually via `workflow_dispatch` as well. Required secrets: `FLY_API_TOKEN` (repo — an **org-scoped deploy token** minted with `flyctl tokens create org personal --expiry 8760h` and stored via `gh secret set FLY_API_TOKEN < tokenfile`; scoped `deploy` / `ssh` tokens fail the pre-flight `appcompact` GraphQL query that `flyctl ssh console` does, and the deprecated `flyctl auth token` output is multi-line so it must be set from a file to avoid stdin newline mangling); `GEMINI_API_KEY` (Fly). The CLI auto-resolves `/data/adai.db` inside the Fly machine via `src/utils/db-path.ts` — no env config needed in the workflow. Every SSH-using step in the workflow first `curl`-warms `/api/stats` because `fly.toml` has `auto_stop_machines = 'stop'` and SSH activity alone doesn't reset the idle timer (the workflow's per-step boundary is just long enough to fall into that window). *(A nightly UMAP re-projection step that fed the now-removed `/embed-space` scatter was retired in the field-flow teardown — June 2026.)*
...
> *(June 2026 — the `/embed-space` UMAP scatter was removed. The 30k `/field` view no longer uses the UMAP projection either: it now lays nodes out in field-flow order (type-runs along the Shape-of-Time draw order) and reveals palette colour only under the cursor. UMAP — the 2D projection only — and its whole production tail (`project_umap.py`, the `umap2d.json` sidecar, the `export-vectors` CLI command, the nightly UMAP cron stage) are gone. The **embeddings themselves** (768-d vectors, STYLE_KIN / VISUALLY_AFFINE / neighbours / Tier-2 propagation) are untouched.)*
...
The Dockerfile is multi-stage: the builder runs `npm run seed:consolidated` and produces `/app/seed.db` (with embeddings + STYLE_KIN + VISUALLY_AFFINE baked in — see the embedding pipeline section); the runtime image ships only that baked DB plus the compiled server. `entrypoint.sh` copies `/app/seed.db → /data/adai.db` on first boot.

`seed/embeddings.{bin,json}` are **committed to git** so they reach the Docker build context. Recreate after editing the seed:
```bash
seed/_build/.venv/bin/python3 seed/_build/embed_nodes.py
git add seed/embeddings.{bin,json} && git commit
```
...
- **Embedding sidecar drift**: if `seed/embeddings.bin` (or `.json`) is missing or out of sync with `seed/nodes.json`, the builder will produce a `seed.db` with no `node_embeddings` rows and no STYLE_KIN / VISUALLY_AFFINE edges (the chained derive silently skips when embeddings are absent). Production then shows empty profile-page similarity sections. Always re-run `embed_nodes.py` after material changes to `seed/nodes.json`, and re-commit the two sidecars. The script is idempotent — only nodes whose `(text_hash, image_hash)` changed get re-embedded.
...
**`cull_orphans.py` — the R2 + embedding janitor.** `seed/_build/cull_orphans.py` finds (a) embedding rows whose node_id left the canon and (b) R2 objects no longer referenced by any cdn (nodes.json + the two image sidecars). Default dry-run; `--apply` rewrites `embeddings.{bin,json}` (recomputing offsets, atomic), `--delete` removes orphan R2 objects (batched, with a refuse-if-<100-referenced bucket-wipe guard). Run `--apply` after any cull so the embeddings stay FK-consistent with the canon (`node_embeddings` has an FK to `nodes` — a stale vector crashes the seeder). May 2026: reclaimed 2.6 GiB of orphan R2 left by the wikidata/refetch experiments.

**`shrink_oversized.py` — the R2 oversized-image janitor (the third sibling).** `seed/_build/shrink_oversized.py` finds R2 objects over a size threshold (default **> 5 MiB**) that are still **referenced by a live node** (it diffs the bucket against `cdn_image_url` in the `--db` live DB, the same reference class `cull_orphans.py` uses under the canon freeze), downloads each, downsizes it so the longest edge is **≤ 2048px** (re-encode quality ~85), and re-uploads it. It **preserves format** (PNG→PNG, GIF→GIF — no lossy surprises), **preserves animation** (animated GIF/WebP: every frame resized), and **never touches `image_url`** (upstream provenance) — only `cdn_image_url` is repointed. It **never upscales** and never goes below the cap ("normal, not too small"), and a resize that doesn't save ≥ `--min-reduction` (10%) is discarded. Because keys are content-addressed, the resized bytes hash to a **new key**, so the tool uploads the new object and writes a **patch JSON** mapping `{node_id → new cdn_image_url}` — it does **not** edit any seed file (frozen) and does **not** delete the original. The repoint lands in the **live prod DB only**, applied by `dist/cli/apply-image-patch.js` over SSH (the only writer; guarded by an old-value match so a drifted prod row is skipped, never clobbered — and it deliberately **does not re-embed**: the image is visually identical, so re-embedding would only churn vectors and spend Gemini). The old oversized object becomes an orphan the instant the DB is repointed → reclaim it with `cull_orphans.py --delete`. **Clean separation: shrink swaps the reference, cull reaps the dead object.** Modes: default = read-only report; `--measure [--sample N]` = real savings on a sample (no writes); `--apply` = resize+upload+write-patch. Wired as `just shrink-oversized-prod` / `-measure` / `-apply` (the apply recipe pulls the live DB, resizes, ssh-applies the patch, then reminds you to cull). ⚠️ the apply recipe needs `dist/cli/apply-image-patch.js` on the machine — `just deploy` first if you just changed it. Typical wins: 4000²/9600² PNG masters → 2048² (70–85% off), oversized JPEGs (95–99% off); 550px animated GIFs that are huge by frame-count shrink ~30% from re-encode alone.
...
- **The trap — dynamic RAISES governance stakes, never lowers them.** The bee
  ...
  back, but live.
- **Embeddings are a separate channel.** `node_embeddings` is a *local* table,
  not a CRR (vectors must not sync as CRDT). A CRDT patch covers the graph; the
  vectors still ride `embed-on-write` + nightly derive. Two coordinated
  channels, not one.
...
# 5. ONLY if the overlay added ARTWORK images: re-embed + re-commit the sidecars.
