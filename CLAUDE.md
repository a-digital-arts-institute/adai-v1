...
**Daily re-derive on Fly**: `.github/workflows/embed-derive-daily.yml` runs `embed:derive` + `embed:report-drift` against the live Fly DB at 04:00 UTC, so curator-approved CREATED_BY edges propagate into the embedding layer without anyone shelling in. Triggered manually via `workflow_dispatch` as well. Requires `FLY_API_TOKEN` repo secret. The CLI auto-resolves `/data/adai.db` inside the Fly machine via `src/utils/db-path.ts` — no env config needed in the workflow.
