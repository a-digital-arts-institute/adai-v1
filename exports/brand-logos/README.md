# A(DAI) Brand Logo Exports

Generated from the brand definitions used by `public/field/brand.html`.

- Font: Courier Prime
- SVG files: `svg/`
- Transparent PNG files: `png/`
- Colors: light `#F2F2F2` and ink `#0C0C0E`
- Monogram PNGs: 4096 x 1024
- Wordmark PNGs: 4096 x 1024
- Primary lockup PNGs: 4096 x 1536

The SVGs are vector source files. The PNGs are rendered through headless Chrome so the web font styling matches the brand page.

## Regenerating

```bash
npm run export:logos
```

The generator (`scripts/export-brand-logos.mjs`) reads the brand system from
`public/field/adai-system.js` and renders each PNG via headless Chrome. It probes
the common macOS/Linux Chrome locations automatically; if your browser lives
elsewhere, set `CHROME_BIN`:

```bash
CHROME_BIN=/path/to/chrome npm run export:logos
```
