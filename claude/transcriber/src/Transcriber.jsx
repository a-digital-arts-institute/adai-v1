...
function LogLine({ text, dim }) {
  return (
    <div style={{ color: dim ? "#A09890" : "#6B9E78", fontFamily: "monospace", fontSize: 13, lineHeight: 1.7 }}>
      {dim ? <span style={{ color: "#B8B0A8" }}>&rsaquo;</span> : <span style={{ color: "#6B9E78" }}>&rsaquo;</span>} {text}
    </div>
  );
}
...
export default function Transcriber() {
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(150deg, #FAF7F2 0%, #F3EFE6 45%, #EBE3D3 100%)",
      color: "#2A2420",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #EDE9E2; } ::-webkit-scrollbar-thumb { background: #C8BFB0; }
        .stage-dot { width: 6px; height: 6px; border-radius: 50%; background: #C8BFB0; transition: background 0.4s; }
        .stage-dot.active { background: #6B9E78; box-shadow: 0 0 8px rgba(107,158,120,0.35); }
        .stage-dot.past { background: #B0D4BA; }
      ...
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        <div style={{ fontSize: 10, letterSpacing: 6, color: "#9E9490", marginBottom: 12, textTransform: "uppercase" }}>
          <span style={{ color: "#6B9E78", marginRight: 8 }}>—</span>A(DAI) Field Intelligence<span style={{ color: "#6B9E78", marginLeft: 8 }}>—</span>
        </div>
        <div style={{ fontSize: 28, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 300, letterSpacing: -1, color: "#2A2420" }}>
          Signal Transcriber
        </div>
      ...
      <div style={{ width: "100%", maxWidth: 560, marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ fontSize: 11, color: "#6B9E78", width: 120, flexShrink: 0 }}>Submitted by</div>
          <select
            value={submittedBy}
            ...
            style={{ maxWidth: 200 }}
          >
            {["Iri", "JB", "Piyush", "Gio", "Guest"].map(name => (
      ...
      >
        ...
        {isRunning ? (
          ...
          <>
            ...
            <div style={{ fontSize: 12, color: "#6E6560" }}>Drop audio/video file (or click to browse)</div>
            <div style={{ fontSize: 11, color: "#A09890", marginTop: 6 }}>MP4, MP3, WAV, M4A, WebM</div>
          </>
        )}
      </div>
              ...
              return (
                <div key={s} style={{ display: "flex", alignItems: "center", flex: i < arr.length - 1 ? 1 : 0 }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                    ...
                    <div style={{ fontSize: 9, color: active ? "#6B9E78" : past ? "#B0D4BA" : "#B8B0A8", letterSpacing: 1, whiteSpace: "nowrap" }}>
                      {s.toUpperCase()}
                    </div>
                  </div>
                  {i < arr.length - 1 && (
                    <div style={{ flex: 1, height: 1, background: past ? "#B0D4BA" : "#D4CCBC", margin: "0 8px 16px" }} />
                  )}
                </div>
              );
            })}
      ...
      {logs.length > 0 && (
        <div style={{
          width: "100%", maxWidth: 560, background: "#EDE9E2",
          border: "1px solid #D4CCBC", borderRadius: 2, padding: "16px",
      ...
      {/* Result */}
      {result && (
        <div style={{ width: "100%", maxWidth: 560, border: "1px solid #B0D4BA", borderRadius: 2, padding: 24, background: "#EBF4EE" }}>
          <div style={{ fontSize: 10, letterSpacing: 4, color: "#6B9E78", marginBottom: 16, textTransform: "uppercase" }}>Signal Posted</div>
          <div style={{ fontSize: 12, color: "#6B9E78", marginBottom: 12 }}>
            <a href={result.pageUrl} target="_blank" rel="noopener noreferrer">&rarr; Open in Notion</a>
          </div>
          <div style={{ fontSize: 11, color: "#6E6560", marginBottom: 8 }}>
            Type: <span style={{ color: "#2A2420" }}>{result.analysis.signal_type}</span>
            &nbsp;&middot;&nbsp;Tags: <span style={{ color: "#2A2420" }}>{result.analysis.tags?.join(", ")}</span>
          </div>
          <div style={{ fontSize: 12, color: "#6E6560", lineHeight: 1.7, borderTop: "1px solid #B0D4BA", paddingTop: 12, marginTop: 12 }}>
